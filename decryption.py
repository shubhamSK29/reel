...
import getpass
try:
    # When used as a package (e.g., python -m package)
    ...
    # When running as a script in the same directory (Flask app)
    from colors import print_colored, Colors
    from crypto import decrypt_password_aes_gcm
    from file_utils import create_file_chooser, read_binary_file
    from steganography import extract_data_from_image
...
def decryption_mode_manual():
    """Legacy: unchanged manual decryption helpers retained from earlier versions."""
    # We'll retain existing manual behaviour from previous code: ask for .bin or base64 inputs.
    from decryption_legacy import decryption_mode_manual as legacy  # if you have a legacy module
    try:
        legacy()
    except Exception:
        print_colored("Legacy manual decryption not available.", Colors.WARNING)


def decryption_mode_web(master_password, image_files):
    """
    Web-compatible decryption function that returns results instead of printing
    """
    import tempfile
    import base64
    ...
    try:
        shares_data = []
        packaged_cipher = None
        
        # Extract shares from uploaded images
        for i, image_file in enumerate(image_files):
            ...
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as temp_file:
                ...
                try:
                    ...
                    payload = extract_data_from_image(temp_file.name)
                    
                    # Parse share payload
                    share_info = _parse_share_payload(payload)
                    
                    shares_data.append({
                        'index': share_info['index'],
                        'share_bytes': share_info['share_bytes'],
                        'total': share_info['total'],
                        'threshold': share_info['threshold']
                    })
                    
                    # Store packaged cipher (should be same in all shares)
                    if packaged_cipher is None:
                        packaged_cipher = share_info['packaged_cipher']
                        
                except Exception as e:
                    # Clean up and continue with other images
        ...
        if not shares_data:
            return {
                'success': False,
                'error': 'No valid shares found in uploaded images'
            }
        
        if packaged_cipher is None:
            return {
                'success': False,
                'error': 'No packaged cipher found in shares'
            }
        
        # Check if we have enough shares
        threshold = shares_data[0]['threshold']
        if len(shares_data) < threshold:
            return {
                'success': False,
                'error': f'Need at least {threshold} shares, but only found {len(shares_data)}'
            }
        
        # Reconstruct the ephemeral key from shares
        share_list = [(s['index'], s['share_bytes']) for s in shares_data]
        K2 = recover_bytes_from_shares(share_list)
        
        # Decrypt the packaged cipher with reconstructed key
        aes = AESGCM(K2)
        nonce2 = packaged_cipher[:12]
        ct_and_tag = packaged_cipher[12:]
        binary_blob = aes.decrypt(nonce2, ct_and_tag, None)
        
        # Extract components from binary blob
        salt = binary_blob[:16]
        nonce = binary_blob[16:28]
        ciphertext_with_tag = binary_blob[28:]
...
def decryption_mode_manual_web(master_password, file_data=None, base64_data=None):
    """
    Web-compatible manual decryption function
    """
    try:
        if file_data:
            # Decrypt from binary file data
            binary_blob = file_data
        elif base64_data:
            # Decrypt from base64 string
            binary_blob = base64.b64decode(base64_data)
        else:
            return {
                'success': False,
                'error': 'No data provided for decryption'
            }
