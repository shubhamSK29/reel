...
def encryption_mode_web(password, master_password, threshold=2, total_shares=3, image_files=None):
    """
    Web-compatible encryption function that returns results instead of printing
    """
    try:
        ...
        # Split K2 into shares
        shares = split_bytes_into_shares(K2, n=total_shares, k=threshold)
        
        # Process each share with provided images
        result_images = []
        for i, share_bytes in enumerate(shares):
            ...
            # Save image to temporary file and embed data
            image_file = image_files[i]
            with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as temp_input:
                image_file.save(temp_input.name)
                
                # Create output filename
                output_filename = f"stego_share_{i+1}.png"
                output_path = os.path.join(tempfile.gettempdir(), output_filename)
                
                # Embed data into image
                saved_path = embed_data_into_image(temp_input.name, payload, output_path=output_path)
                
                # Read the result image as base64 for web transfer
